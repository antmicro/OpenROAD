...
const char* dbNetwork::pathName(const Net* net) const
{
  ...
  if (modnet) {
    ...
    const char* modnet_name_char = modnet_name.c_str();
    full_path_buf.append(modnet_name_char, modnet_name_char + std::strlen(modnet_name_char));
    ...
    const std::set<std::string>& names2)
{
  ...
  std::set_difference(names1.begin(),
                      names1.end(),
                      names2.begin(),
                      names2.end(),
...
void dbNetwork::checkSanityModInstTerms() const
{
  for (odb::dbModInst* mod_inst : block_->getModInsts()) {
    ...
    if (master) {
      ...
      // be in iterms.
      bterms_only.erase(
          std::remove_if(bterms_only.begin(),
                         bterms_only.end(),
