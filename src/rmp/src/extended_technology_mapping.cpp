...
static std::tuple<mockturtle::names_view<mockturtle::aig_network>,
                  cut::LogicCut>
create_mockturtle_aig(sta::dbSta* sta,
                      sta::Corner* corner,
                      rsz::Resizer* resizer,
                      utl::Logger* logger)
{
  ...
  utl::UniquePtrWithDeleter<Abc_Ntk_t> mapped_abc_network
      = cut.BuildMappedAbcNetwork(abc_library, network, logger);

  utl::UniquePtrWithDeleter<Abc_Ntk_t> current_network(
      Abc_NtkToLogic(const_cast<Abc_Ntk_t*>(mapped_abc_network.get())),
      &Abc_NtkDelete);

  {
    auto library = static_cast<Mio_Library_t*>(mapped_abc_network->pManFunc);

    // Install library for NtkMap
    Abc_FrameSetLibGen(library);

    current_network->pManFunc = library;

    {
      abc_ntk = current_network.get();
      assert(!Abc_NtkIsStrash(ntk));
      // derive comb GIA
      auto strash = Abc_NtkStrash(abc_ntk, false, true, false);
  ...
  node_to_sig[Aig_ManConst0(aig)] = const0;
  ...
  {
    auto pCi = Abc_NtkPi(abc_ntk, i);
    auto s = ntk.create_pi(Abc_ObjName(pCi));
  ...
  {
    auto* pF0 = Aig_ObjFanin0(pObj);
    auto* pF1 = Aig_ObjFanin1(pObj);
  ...
  // Primary outputs (COs)
  Aig_ManForEachCo(aig, pObj, i)
  {
    auto pCo = Abc_NtkPo(abc_ntk, i);
    auto* pF = Aig_ObjFanin0(pObj);
    auto s = node_to_sig.at(pF);

    if (Aig_ObjFaninC0(pObj)) {
      s = ntk.create_not(s);
    }
    ntk.create_po(s, Abc_ObjName(pCo));
  }
...
static void import_mockturtle_mapped_network(sta::dbSta* sta,
                                             const BlockNtk& ntk,
                                             odb::dbLib* lib,
                                             cut::LogicCut& cut,
                                             utl::Logger* logger)
{
  ...
  auto node_index = [&](const BlockNtk::node& n) -> std::size_t {
    return static_cast<std::size_t>(topo_ntk.node_to_index(n));
  };
  ...
  topo_ntk.foreach_gate([&](const BlockNtk::node& n) {
    ...
    odb::dbMaster* master = lib->findMaster(mapping.master_name.c_str());
    if (!master) {
      throw std::runtime_error(
    ...
    for (uint32_t out_pin_idx = 0; out_pin_idx < num_node_outputs;
      ...
      if (!net) {
        throw std::runtime_error(
            fmt::format("Failed to create net {}", net_name));
      }
    ...
    topo_ntk.foreach_fanin(n, [&](const BlockNtk::signal& f) {
      ...
      if (topo_ntk.is_constant(src_node)) {
        ...
        if (src_idx >= node_out_nets.size()
          ...
          throw std::runtime_error(
              fmt::format("Missing driver net for fanin of node {}", idx));
        }
                                 ...
                                 utl::Logger* logger)
{
  ...
  auto [ntk, cut] = create_mockturtle_aig(sta, corner, resizer, logger);
  ...
  import_mockturtle_mapped_network(sta, mapped_ntk, lib, cut, logger);
