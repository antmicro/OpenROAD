...
void resynth_emap_cmd(Corner* corner, char* target, bool map_multioutput, float wireload_penalty, bool verbose, char* workdir_name) {
  getRestructure()->setMode(target);
  getRestructure()->resynthEmap(corner, map_multioutput, wireload_penalty, verbose, workdir_name);
}
